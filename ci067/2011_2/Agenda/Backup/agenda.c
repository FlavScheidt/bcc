...
/***********************
	Comentários Gerais:
		- Arrumar falha na leitura da última ID inserida
		- Arrumar a busca
		- Fazer as funções de altera e exclui (pesquisar)
************************/
/***********************
	Última Alteração: 21/Out/2011 18:00
************************/
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
char * Le(int tam) //Lê string da linha de comando para uma variável temporária, aloca espaço exato para a variável
{
...
void InsereContato(char *agenda, int *ultimaID) //Insere contato no arquivo
{
	...
	/* Email */
	printf("E-mail: ");
	contato.email = Le(200);
	/* Telefone */
	printf("Telefone: ");
	contato.telefone = Le(25);
	/* ID */
	*ultimaID = *ultimaID+1;
	contato.ID = *ultimaID;
	/***** Joga no arquivo *****/
